# don't forget to install send2trash
# pip install send2trash

#python cleanup_trec.py "D:\Development\bl_CommandBox\tmp" --recursive --sendtotrash --all-unused
                    ...
                    for track in source['sourceTracks']:
                        if 'metaData' in track:
                            ...
                            if isinstance(metadata, str) and ';' in metadata:
                                ...
                                for file in files:
                                    if file.strip() and file.strip().lower(
                                    ).endswith('.trec'):
...
def process_directory_or_file(path,
    ...
    if os.path.isdir(path):
        ...
        if os.path.isfile(potential_tscproj):
            ...
            print(
                f"Found matching tscproj file: {os.path.basename(tscproj_path)}"
            )
        else:
            # Look for any tscproj file in the directory
            tscproj_files = [
                f for f in os.listdir(directory)
                if f.lower().endswith('.tscproj')
            ]
            if tscproj_files:
                tscproj_path = os.path.join(directory, tscproj_files[0])
                print(
                    f"Using first tscproj file found: {os.path.basename(tscproj_path)}"
                )
            else:
    ...
    if all_unused:
        ...
        for file in all_files:
            if file not in referenced_files and not file.lower().endswith(
                    '.json'):
    ...
    if not files_to_delete:
        ...
        if all_unused:
            ...
            print(
                f"Found {len(files_to_delete)} unused .trec files in {directory}:"
            )
...
def process_recursively(base_path, dry_run, all_unused, list_used):
    ...
    if os.path.isfile(base_path) and base_path.lower().endswith('.tscproj'):
        if process_directory_or_file(base_path, dry_run, all_unused,
                                     list_used):
            processed_count += 1
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for tscproj_file in tscproj_files:
            ...
            if process_directory_or_file(tscproj_path, dry_run, all_unused,
                                         list_used):
                processed_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Clean up unused .trec files in Camtasia project files')
    parser.add_argument(
        'path',
        help=
        'Path to a directory containing a .tscproj file or to the .tscproj file itself'
    )
    parser.add_argument(
        '--sendtotrash',
        action='store_true',
        help='Send unused files to trash (otherwise just report)')
    parser.add_argument(
        '--all-unused',
        action='store_true',
        help='Send all unused files (not just .trec files) to trash')
    parser.add_argument('--list-used',
                        action='store_true',
                        help='List all files used in the project')
    parser.add_argument(
        '--recursive',
        action='store_true',
        help='Recursively process all .tscproj files in subdirectories')

    if (len(sys.argv) - 1) == 0:
        print(f'eg: python cleanup_trec.py "D:\\Development\\CamtasiaVideos\\tmp" --recursive --sendtotrash --all-unused')
        return
    ...
    if args.recursive:
        ...
        count = process_recursively(args.path,
                                    dry_run=not args.sendtotrash,
                                    all_unused=args.all_unused,
                                    list_used=args.list_used)
